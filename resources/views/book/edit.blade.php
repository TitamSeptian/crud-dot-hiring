...
@section('content')
    ...
    <form action="{{ route('book.update', $book->id) }}" method="post" enctype="multipart/form-data">
        ...
        <div class="form-control">
            ...
            <input class="input" type="text" name="title" id="title" value="{{ old('title') ?? $book->title }}"
        ...
        <div class="form-control">
            ...
            <input class="input" type="text" name="author" id="author" value="{{ old('author') ?? $book->author }}"
                required autocomplete="off" autofocus>
            @error('author')
                <span class="invalid">{{ $message }}</span>
            @enderror
        ...
        <div class="form-control sm:col-span-2 mt-2">
            ...
            <textarea class="input" type="text" name="description" id="description" value="{{ old('description') }}" required
                autocomplete="off" autofocus rows="30" cols="50">{{ old('description') ?? $book->description }}</textarea>
            @error('description')
                <span class="invalid">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="aspect-[3/4] rounded-xl overflow-hidden w-full bg-gray-100">
            <img class="w-full h-full object-cover " src="{{ \Storage::url($book->image) }}" id="preview-image">
        </div>
    </form>
@endsection
